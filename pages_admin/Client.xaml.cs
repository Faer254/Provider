...
namespace Provider.pages_admin
{
    public partial class Client : Page
    {
        ...
        {
            if (!(bool)new QuestionWindow("Вы уверены, что хотите удалить данного клиента и все его подключенные услуги?", Manager.theme).ShowDialog())
            {
                return;
            }
            ...
            if (!Manager.dataBase.completeCommand($"BEGIN; SET FOREIGN_KEY_CHECKS = 0; DELETE FROM `concluded_services` WHERE `client_id` = {dataFromRow[0]}; DELETE FROM `client` WHERE `id` = {dataFromRow[0]}; SET FOREIGN_KEY_CHECKS = 1; COMMIT;"))
            {
                _employeeWindow.loadEmployee();
                return;
            }

            Manager.dataBase.addEmployeeAction((uint)Manager.user[0], $"Удалил клиента id {dataFromRow[0]} ({dataFromRow[4]}) и все подключённые услуги", DateTime.Now);
