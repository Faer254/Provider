...
namespace Provider.pages_admin
{
    public partial class AccountSettings : Page
    {
        ...
        private void editFullNameButton_Click(object sender, RoutedEventArgs e)
        {
            newLoginTB.IsEnabled = false;
            newEmailTB.IsEnabled = false;
            acceptCodeTB.IsEnabled = false;
            ...
            newPhoneTB.Mask = "";
            acceptCodePhoneTB.IsEnabled = false;
            newFullNameTB.IsEnabled = true;

            newLoginTB.Text = string.Empty;
            newEmailTB.Text = string.Empty;
            acceptCodeTB.Text = string.Empty;
            acceptCodePhoneTB.Text = string.Empty;
            newFullNameTB.Text = (string)Manager.user[4];

            saveLoginButton.IsEnabled = false;
            saveEmailButton.IsEnabled = false;
            savePhonenButton.IsEnabled = false;
            saveFullNameButton.IsEnabled = true;

            newFullNameTB.Focus();
            newFullNameTB.CaretIndex = newFullNameTB.Text.Length;
        }
        ...
        {
            if (newFullNameTB.Text.ToLower() == ((string)Manager.user[4]).ToLower() || newFullNameTB.Text == string.Empty)
            {
                newFullNameTB.IsEnabled = false;
                newFullNameTB.Text = string.Empty;
                saveFullNameButton.IsEnabled = false;
            ...
            string fullName = newFullNameTB.Text.Trim();
            string[] parts = fullName.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);

            if (parts.Length != 3)
            {
                employeeWindow.ShowAndHideError("Неверно указано ФИО!");
                return;
            }
            ...
            if (!Manager.dataBase.updateEmployee((uint)Manager.user[0], "full_name", fullName))
            {
                employeeWindow.loadEmployee();
                return;
            }

            Manager.dataBase.addEmployeeAction((uint)Manager.user[0], $"Изменил ФИО \"{Manager.user[4]}\" -> \"{fullName}\"", DateTime.Now);
            loadData();
            employeeWindow.loadEmployee();
        ...
        private void newFullNameTB_PreviewTextInput(object sender, TextCompositionEventArgs e)
        {
        ...
        private void newFullNameTB_TextChanged(object sender, TextChangedEventArgs e)
        {
            while (newFullNameTB.Text.Count(c => c == ' ') >= 3)
            {
                newFullNameTB.Text = newFullNameTB.Text.Remove(newFullNameTB.Text.Length - 1);
                newFullNameTB.CaretIndex = newFullNameTB.Text.Length;
            }
        }
        ...
        private void Page_KeyUp(object sender, KeyEventArgs e)
        {
            switch (e.Key)
            {
                ...
                case Key.Return when newFullNameTB.IsFocused:
                    saveFullNameButton_Click(sender, e);
                    break;
